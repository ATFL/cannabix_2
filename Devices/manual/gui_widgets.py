...
class LiveGraph(tk.Frame):
    ...
    def addData(self, xVal, yVal):
        ...
        self.fig.clear()
        self.fig.add_subplot(111).plot(self.xList, self.yList)
        self.canvas.draw()
...
class Timer(tk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self.now = 0
        self.label = tk.Label(self, text=str(self.now)+' s',font=20)
        self.label.place(relx=0,rely=0,relheight=0.3,relwidth=.3)
        ...
        stopbtn = tk.Button(self, text='Stop')
    def update(self):
        self.now += 1
