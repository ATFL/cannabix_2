...
# Pump
pinPump = 11 #Darius Edit 
pump = Pump(pinPump)
#################### System Variables ####################
# Purging Variables
clean_chamber_purge_time = 65 # normally 30s
sensing_chamber_purge_time = 65 # normally 60s
# Filling Variables
...
duration_of_signal = 120 # normally 150, time allowed for data acquisition per test run
#################### Data Array ####################
...
def purge_system():
    ...
    #darius edit was if linearActuator.state != 'retracted':
    #darius edit was    linearActuator.retract()      
    ...
    while ((time.time() < (start_time + sensing_chamber_purge_time)) and continueTest == True):
        ...
        #darius was if linearActuator.state != 'retracted':
        #darius was   linearActuator.retracted()
    ...
    # DArius Clean chamber Purge the sensing chamber.
    ...
    while ((time.time() < (start_time + clean_chamber_purge_time)) and continueTest == True):
        messagebox.showinfo("loop2","loop")
        if pump.state != True:
             pump.enable()
        if linearActuator.state != 'extended':
            linearActuator.extended()
            
        #darius if linearActuator.state != 'extended':
        #darius    linearActuator.extend()
...
def collect_data(xVector,yVector):
    ...
    while (time.time() < (start_time + duration_of_signal)) and (continueTest == True):  # While time is less than duration of logged file
        ...
        # If time is between 10-50 seconds and the Linear Actuator position sensor signal from the ADC indicates a retracted state, extend the sensor
            ...
            if pump.state != False: #DARIUS EDIT
                pump.disable() #DARIUS EDIT
       #DARIUS EDIT out
       ...
       #DARIUS EDIT If time is greater than 120 seconds extend actuator and fans on
       # elif (time.time() > (duration_of_signal + start_time)) and (continueTest == True):
       #     if linearActuator.state != 'extended':
       #       linearActuator.extended()
        #    if pump.state != True:
         #     pump.enable()
        ...
        # Otherwise, keep outputs off
            ...
            #DARIUS EDITif linearActuator.state != 'retracted':
            #DARIUS EDIT   linearActuator.retract()
                if linearActuator.state != 'default': #DARIUS EDIT
                        linearActuator.default() #DARIUS EDIT
                #if pump.state != True: #DARIUS EDIT
