...
def ADC_linear_actuator():
    conversion_value = (adc1.read_adc(0,gain=GAIN)/pow(2, 15))*6.144
...
try: 
    ...
    window = Tk()
    w, h = 640, 480
    window.title("Pressure Breath Testing")
    left = Frame(window, borderwidth = 2, relief = "solid")
    left.pack(side = "left", expand = True, fill = "both")
    newGraph1 = Figure()
    newGraph1.set_size_inches(5,1.5,forward=True)
    newPlot1 =  newGraph1.add_subplot(111)
    ...
    sensing_retract_time = 130 # normally 130 time allowed before sensor is retracted, no longer exposed to sample
    duration_of_signal = 300 # normally 300time allowed for data acquisition per test run
    ...
    dataVector5 = [] # data values to be returned from the pressure sensor
    ...
    newPlot1.plot(dataVector1)
    newPlot1.plot(dataVector2)
    newPlot1.plot(dataVector3)
    newPlot1.plot(dataVector4)
    newPlot1.plot(dataVector5)
    newCanvas = FigureCanvasTkAgg(newGraph1, left)
    newCanvas.draw()
    newCanvas.get_tk_widget().pack(fill=BOTH, expand = True)
    ...
    while (time.time() < (start_time + duration_of_signal)) and (continueTest == True):  # While time is less than duration of logged file
        if (time.time() > (start_time + (
            ...
            dataVector4.append(adc1.read_adc(3,
                                            gain=GAIN))  # Perform analog to digital function, reading voltage from second sensor channel
            dataVector5.append(adc2.read_adc(1, gain = GAIN))
            newPlot1.clear()
            newPlot1.plot(dataVector1)
            newPlot1.plot(dataVector2)
            newPlot1.plot(dataVector3)
    ...
    combinedVector = np.column_stack((timeVector, dataVector1, dataVector2, dataVector3, dataVector4, dataVector5
                                      ))
